...
<TutoPopup bind:popup={tuto1} info="Ce bouton est cliquable" memorisation="Je peux cliquer sur le bouton auquel le popup fait référence, c'est à dire celui dont l'encadrement est rouge">
	<button onclick={()=>alert(faits[Math.floor(Math.random() * faits.length)])}>Obtenir une information inutile</button>
</TutoPopup>

<TutoPopup bind:popup={tuto2} info="Ce bouton est cliquable" memorisation="Je peux cliquer sur le bouton auquel le popup fait référence, c'est à dire celui dont l'encadrement est rouge, et qui n'affiche pas le nombre {Math.random().toString()}">
